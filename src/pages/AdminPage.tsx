...
export default function AdminPage() {
  ...
  // 1. REPLACEMENT: Simple check instead of Supabase Session
  useEffect(() => {
    const isAuthenticated = localStorage.getItem("admin_authenticated");
    
    if (isAuthenticated !== "true") {
      // If the flag isn't there, kick them to login
      navigate("/auth");
    }
  }, [navigate]);
  ...
  // 3. Removed the "Loading..." spinner since the check is instant now
    ...
    <div className={cn("flex flex-col h-full", className)}>
      ...
      <div className="p-4 border-t border-border space-y-2">
        <div className="flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground">
          {/* Hardcoded Name since we don't have a database user anymore */}
